...
void *mqtt_thread(void *p)
{
	...
	if (mosq)
	{
		...
		mosquitto_publish_callback_set(mosq, publish_callback);//发布回调
		mosquitto_message_callback_set(mosq, message_callback);//订阅回调
		...
		rc = mosquitto_subscribe(mosq, NULL, "#", 0);//订阅
		if (rc == MOSQ_ERR_SUCCESS)
		{
		...
		rc = mosquitto_publish(mosq, NULL, "pub/qos1/test", strlen("message"), "message", 1, false);//发布
		if (rc == MOSQ_ERR_SUCCESS)
		{
