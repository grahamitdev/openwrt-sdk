/*************************************************************************
  ...
  > Created Time: 2015年05月20日 星期三 16时42分13秒
 ************************************************************************/
...
{
	uint8_t channel;
	struct blob_attr *tb[__ZHA_GATEWAY_ATTR_MAX]; //tb就是一个：结构体(指针)数组，的名子，tb就是一个普通数组的数组名
					   ...
					   struct blob_attr *msg)
{
	...
	char ieeeaddr_string[17]; //1个字符（1个字节）显示一个hex+48
	coordInfo_t coordInfo;

	coord_getInfo(&coordInfo);
	//获取协调器信息
	blob_buf_init(&b, 0);
	//ieeeAddr实际64位bin   uint8类型：ieeeAddr[n]:0:0000 0000:00:需要两个char来存,|48|48|...|...|...|...|...|...|
...
Zstatus_t zha_list_backinfo(deviceInfo_t *device_info, uint16_t num) //这些信息由网关SDK内部函数提供，这些回调函数只负责打印信息
{
		...
		__blob_for_each_attr(pattr, blobmsg_data(tb[GROUP_ATTR_DEVICE]), len)
		{
			...
			if (wttb[ZHA_GATEWAY_ATTR_ID])
			{
				...
				if (i < 100)
				{
					...
					hexStr2bytes(blobmsg_get_string(wttb[ZHA_GATEWAY_ATTR_ID]), groupMember[member_count].ieeeAddr, 8);
		...
		__blob_for_each_attr(pattr, blobmsg_data(tb[RULE_ATTR_ACTIONS]), group_id)
		{
			...
			if (subActTb[RULE_ATTR_ACT_CMD])
			{
				if (subActTb[RULE_ATTR_ACT_VALUE])
				{
					...
					else if ((0 == memcmp(blobmsg_get_string(subActTb[RULE_ATTR_ACT_CMD]), "bri", strlen("bri"))) || (0 == memcmp(blobmsg_get_string(subActTb[RULE_ATTR_ACT_CMD]), "hue", strlen("hue"))) || (0 == memcmp(blobmsg_get_string(subActTb[RULE_ATTR_ACT_CMD]), "sat", strlen("sat"))) || (0 == memcmp(blobmsg_get_string(subActTb[RULE_ATTR_ACT_CMD]), "ctp", strlen("ctp"))))
					{
	...
	if (rule)
	{
		...
		while (tmp_conditions)
		{
			...
			case 2:
			{
				...
				else if ((0 == memcmp(tmp_conditions->rule_cond.triggered_t.attrName, "bri", strlen("bri"))) || (0 == memcmp(tmp_conditions->rule_cond.triggered_t.attrName, "hue", strlen("hue"))) || (0 == memcmp(tmp_conditions->rule_cond.triggered_t.attrName, "sat", strlen("sat"))) || (0 == memcmp(tmp_conditions->rule_cond.triggered_t.attrName, "colortemp", strlen("colortemp"))))
				{
		...
		while (tmp_actions)
		{
			...
			else if ((0 == memcmp(tmp_actions->attrName, "bri", strlen("bri"))) || (0 == memcmp(tmp_actions->attrName, "hue", strlen("hue"))) || (0 == memcmp(tmp_actions->attrName, "sat", strlen("sat"))) || (0 == memcmp(tmp_actions->attrName, "colortemp", strlen("colortemp"))))
			{
				...
				struct blob_attr *msg)
{
	...
	blob_buf_init(&b, 0);
	blobmsg_add_string(&b, "versions", SHUNCOM_SDK_VER);
