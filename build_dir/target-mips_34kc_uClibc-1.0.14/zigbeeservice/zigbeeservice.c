...
int local_find_device(unsigned short profile_id, unsigned short device_id)
{
	...
	//先判断设备基于什么物联网协议
	if (ZCL_HA_PROFILE_ID == profile_id) //ZHA
		profile = HA_PROFILE_DEVICE;
	else if (ZLL_PROFILE_ID == profile_id) //ZLL
		profile = ZLL_PROFILE_DEVICE;
	else if (ZCL_SHUNCOM_PROFILE_ID == profile_id) //SHUNCOM
...
//当zigbee网关下的节点状态发生改变时,SDK调用回调函数，回调函数调用这个函数，在终端打印变化节点的信息
int local_list_new(cJSON **r_result)
{
	...
	uint8_t ieeeAddr[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; //真实单个ieeeAddr数组格式
	char ieeeaddr_string[17] = {0};											//单个ieeeAddr字节数组（int->ascii）,便于打印和传输
	...
	//获取协调器信息
	//一个宏函数，遍历当前网关下的所有节点信息，无返回值
	//device_table_head 挂载有设备信息的avl树的首节点
	//device 类型device_info_t* 返回单个设备的详细信息！
	//avl avl树的节点成员
...
//这个函数1s被调用一次，目的是把新入网的设备加到白名单列表里，1s中来得及遍历所有当前结点
void write_device_in_white_list(void)
{
	...
	uint8_t ieeeAddr[200 * 8] = {0}; //200个8*8,能存200个IEEE地址,单个设备定义为拥有相同IEEE地址的"所有端口",有的节点上有多个端口
	char ieeeaddr_string[17] = {0};
	deviceInfo_t *devices = NULL;
	//实时获取当前网关下所有节点的(64位ieee)，返回设备个数
	numOfDevices = device_getDevicesList_fill(&devices);//也可用avl_for_each_element_safe(&(device_table_head.hdr),device,avl,pdevice)
	//从数据库中获取当前网网关下公共的白名单列表，返回白名单里节点个数，保存设备64位ieee地址到数组中
	num_ieeeAddr = device_getWhitelist(ieeeAddr);//存到数组里
	//每个节点遍历公共的白名单列表，看该节点在不在白名单列表里，其实这里有点浪费资源
	for (i = 0; i < numOfDevices; i++) //遍历节点
	{
		for (j = 0; j < num_ieeeAddr; j++) //该节点遍历公共白名单列表
		{
			if (memcmp(devices[i].deviceBasic.ieeeAddr, ieeeAddr + j * 8, 8) == 0) //两个指针同时向右移动64位，步长64(8字节)
			{
				find_flag = 1; //该节点在白名单里
				break;
			}
		}
		//不管在不在白名单，都把该节点的IEEE地址打出来
		snprintf(ieeeaddr_string, 17, "%02x%02x%02x%02x%02x%02x%02x%02x",
...
void *cloud_thread(void *p)
{
	...
	//首先关闭白名单列表
	//然后开放所有zigbee节点许可60秒permit join
	...
	while (1)
	{
		...
		case 2:
		{
			...
			memset(ieeeaddr, 0xff, 8);			//面向所有zigbee地址
			device_setPermitJoin(ieeeaddr, 60); //允许节点入网，持续时间：60s
		...
		case 60: //60s后，使能白名单
		{
			...
			return NULL; //是能后，退出循环
			break;
		}
		default: //在60s内，把当前网关下的所有节点加到白名单里
		{
...
#endif
	zigbee_loop(); //事件循环主入口(适当时机会调用回调函数)
	return 0;
}
