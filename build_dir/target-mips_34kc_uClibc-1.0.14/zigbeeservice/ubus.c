...
const struct blobmsg_policy zha_gateway_attrs[__ZHA_GATEWAY_ATTR_MAX] = {
	...
	[ZHA_GATEWAY_ATTR_WHITELIST] = {"whitelist", BLOBMSG_TYPE_ARRAY},
...
//宏展开
//{{.name = "list",.handler = zha_list},{},{},{},{},{}...}
//封装函数到结构体里
//再封装成结构体数组
static const struct ubus_method zha_methods[] = {
...
struct ubus_object_type zha_object_type =
	UBUS_OBJECT_TYPE("zha", zha_methods);

struct ubus_object zha_object = {
	.name = "zha",
	.type = &zha_object_type,
	.methods = zha_methods,
	.n_methods = ARRAY_SIZE(zha_methods),
};
...
void ubus_init()
{
	...
	ctx = ubus_connect(ubus_socket);//2
	if (!ctx)
	{
		fprintf(stderr, "Failed to connect to ubus\n");
	}

	ret = ubus_add_object(ctx, &zha_object);//注册方法
	if (ret)
		fprintf(stderr, "Failed to add object: %s\n", ubus_strerror(ret));

	ubus_add_uloop(ctx);//3事件循环
}
